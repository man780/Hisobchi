...
<link href="style/css/modal.css" rel="stylesheet" type="text/css" />
    ...
    include("blocks/bd.php"); 
        ...
        $result21 = mysql_query("SELECT * FROM chiqim WHERE id=".$id,$db);
		...
		<h3 class='headline3'>Yangi chiqim qo`shish</h3>		
		 <form action="add_chiqim.php" method="post" name="form_chiqim" id="questions">		
            ...
            //echo $myrow13["name"];
				do{
				    //echo $myrow21["cat_c"];                    
				    //if($myrow21["cat_c"]==$myrow13["name"])
                    echo "<option value='$myrow21[cat_c]'>$myrow21[cat_c]</option>";
			...
			<p><label>Ost Kategoriya:</label><br><select name="podcat_c">
				<option selected value='<? echo $myrow21["podcat_c"];?>'></option>
			<?
			...
			<p><label>Summa:</label> <br><input type="text" name="sum_c" value="<? echo $myrow21["sum_c"];?>"/></p>
			...
			<p><input name="sub_com" type="submit" value="Chiqim o`zgartirish" />
